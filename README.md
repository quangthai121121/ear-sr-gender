...
└── data/
    ...
    │       └── images/
    │           ├── 01/*.jpg     (subject 01, male)
    │           ├── ...
    │           ├── 98/*.jpg     (last male subject)
    │           ├── 99/*.jpg     (first female subject)
    │           ├── ...
    │           └── 164/*.jpg    (last female subject)
    ├── processed/                <- Real-ESRGAN/SwinIR x4 outputs cached here
    │   ├── realesrgan/<subject>/<file>.png
    │   └── swinir_large/<subject>/<file>.png
...
So the simplest setup is: download EarVN1.0, extract it so you get
`data/raw/EarVN1.0/images/01/`, `.../02/`, ... `.../164/`, and **you don't
need to touch `configs/paths.yaml` at all.**
...
```yaml
paths:
  data_root: "/mnt/fastdisk/EarVN1.0"           # must contain images/01 .. images/164
