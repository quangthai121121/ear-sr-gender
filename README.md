...
## 1. Where data goes, and the ONE file you edit: `configs/paths.yaml`

By default, this project keeps everything **inside its own `data/` folder**
— no external system paths needed:

```
ear-sr-gender/
└── data/
    ├── raw/
    │   └── EarVN1.0/            <- EXTRACT THE DATASET HERE
    │       ├── 001.ALI_HD/*.jpg       (subject 1, male)
    │       ├── 002.LeDuong_BL/*.jpg   (subject 2, male)
    │       ├── ...
    │       ├── 098.<name>/*.jpg       (last male subject)
    │       ├── 099.<name>/*.jpg       (first female subject)
    │       ├── ...
    │       └── 164.<name>/*.jpg       (last female subject)
    ├── processed/                <- Real-ESRGAN/SwinIR x4 outputs cached here
    │   ├── realesrgan/<same subfolder names as above>/<file>.png
    │   └── swinir_large/<same subfolder names as above>/<file>.png
    └── meta/                      <- generated by scripts/03 and 04
        ├── images.csv
        ├── subjects.csv
        └── splits/
```

`build_metadata.py` auto-detects your layout: subject folders can sit
directly under `data_root` (as above) or under `data_root/images/`, and
can be named `"01"`, `"001_"`, `"001.ALI_HD"`, etc. — only the **leading
number** in the folder name is used to determine `subject_id` and gender
(subjects 1-98 = male, 99-164 = female, per EarVN1.0's own docs). If your
copy orders subjects differently, check `data/meta/subjects.csv`'s
`folder_name` column after running step 5 below, and adjust
`subject_gender()` in `src/data/build_metadata.py` if needed.

So the simplest setup is: download EarVN1.0, extract it so its subject
folders land directly under `data/raw/EarVN1.0/`, and **you don't need to
touch `configs/paths.yaml` at all.**

If you'd rather keep the (often large) dataset or SR cache outside the
repo — e.g. on a separate fast/large disk — open `configs/paths.yaml` and
point `data_root` / `processed_root` at an absolute path instead:

```yaml
paths:
  data_root: "/mnt/fastdisk/EarVN1.0"           # your subject folders live directly under here (or under an images/ subfolder)
  processed_root: "/mnt/fastdisk/EarVN1.0_processed"
```

Relative paths (starting with `./`) resolve against the repo root;
absolute paths (starting with `/`) are used as-is. Everything else
(`meta_root`, checkpoint paths, results paths) defaults to locations
inside this repo and normally doesn't need to change.

If your Mendeley download's `images/` subfolder is numbered differently
than `01..164`, or if 98/66 male/female no longer matches, fix
...
# 3. Scan data_root (auto-detects layout) -> data/meta/images.csv, subjects.csv
